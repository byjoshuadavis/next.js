...
- `query`: `Object` - The query string parsed to an object. It will be an empty object during prerendering if the page doesn't have [data fetching requirements](/docs/basic-features/data-fetching/overview.md). Defaults to `{}`. The query contains paramters from [dynamic paths](https://nextjs.org/docs/routing/dynamic-routes) and query strings.
